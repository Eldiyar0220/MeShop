...
class MainPageView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        filter = self.request.GET.get('filter')
        if search:
            context['productes'] = Products.objects.filter(Q(title__icontains=search)|
                                                       Q(description__icontains=search))
        elif filter:
            start_date = timezone.now() - timedelta(days=1)
            context['productes'] = Products.objects.filter(created__gte=start_date)
        else:
            context['productes'] = Products.objects.all()
        return context
...
class Category_detail_view(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['productes'] = Products.objects.filter(category_id=self.slug)
        return context
...
def update_recipe(request, pk):
    recipe = get_object_or_404(Products, pk=pk)
